...
<template>
  ...
  >
    <div class="max-w-md w-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl p-5 sm:p-6 text-center shadow-2xl relative overflow-hidden flex flex-col items-center animate-scaleUp">
      ...
      <div class="w-full bg-gray-50 dark:bg-slate-800/70 border border-gray-200 dark:border-slate-700/70 rounded-2xl p-5 sm:p-6 flex flex-col items-center shadow-inner relative">
        
        <!-- Character Display -->
        <div 
          class="font-black text-gray-900 dark:text-amber-300 drop-shadow-sm mb-1 font-jp leading-tight text-center select-all"
        ...
        <div class="flex items-center justify-center gap-2.5 my-1.5 flex-wrap">
          <span class="text-xl sm:text-2xl font-black text-indigo-600 dark:text-indigo-400 tracking-wider uppercase">
            {{ Array.isArray(item.romaji) ? item.romaji.join(' / ') : item.romaji }}
          </span>
          <button 
            @click="playAudio"
            class="p-2 rounded-full bg-indigo-100 hover:bg-indigo-200 dark:bg-indigo-500/20 dark:hover:bg-indigo-500/40 text-indigo-600 dark:text-indigo-300 transition active:scale-95 cursor-pointer shadow-xs"
